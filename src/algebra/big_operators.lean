...
@[to_additive is_add_monoid_hom.map_sum]
theorem is_monoid_hom.map_prod (f : α → β) [is_monoid_hom f] (l : list α) :
  f (prod l) = prod (map f l) :=
by induction l; simp only [*, is_monoid_hom.map_mul f, is_monoid_hom.map_one f, prod_nil, prod_cons, map]

theorem is_group_anti_hom.map_prod {α β} [group α] [group β] (f : α → β) [is_group_anti_hom f] (l : list α) :
...
theorem inv_prod {α β} [group α] [group β] :
  ∀ l : list α, (prod l)⁻¹ = prod (map (λ x, x⁻¹) (reverse l)) :=
λ l, @is_group_anti_hom.map_prod _ _ _ _ _ inv_is_group_anti_hom l -- TODO there is probably a cleaner proof of this

end group

section comm_monoid
variables [comm_monoid α] [comm_monoid β] (f : α → β) [is_monoid_hom f]

@[to_additive is_add_monoid_hom.multiset_sum]
lemma is_monoid_hom.map_multiset_prod (m : multiset α) : f m.prod = (m.map f).prod :=
quotient.induction_on m $ assume l, by simp [is_monoid_hom.map_prod f l]

@[to_additive is_add_monoid_hom.finset_sum]
lemma is_monoid_hom.finset_prod (g : γ → α) (s : finset γ) : f (s.prod g) = s.prod (f ∘ g) :=
show f (s.val.map g).prod = (s.val.map (f ∘ g)).prod,
  by rw [is_monoid_hom.map_multiset_prod f]; simp
