...
variables (f : α →r β) {x y : α}

lemma map_ne_zero : f x ≠ 0 ↔ x ≠ 0 :=
⟨mt $ λ h, h.symm ▸ f.map_zero,
 λ x0 h, one_ne_zero $ calc
    1 = f (x * x⁻¹) : by rw [mul_inv_cancel x0, f.map_one]
  ... = 0 : by rw [f.map_mul, h, zero_mul]⟩
...
by rw [mul_inv_cancel ((map_ne_zero f).2 h), ← f.map_mul, mul_inv_cancel h, f.map_one]

lemma map_div' (h : y ≠ 0) : f (x / y) = f x / f y :=
(f.map_mul _ _).trans $ congr_arg _ $ f.map_inv' h

lemma injective : function.injective f :=
(add_monoid_hom.injective_iff (f : α →0+ β)).2
  (λ a (ha : f a = 0), classical.by_contradiction $ λ ha0,
...
(map_mul f _ _).trans $ congr_arg _ $ map_inv f
