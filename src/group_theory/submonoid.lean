...
/-- The image a submonoid under an isomorphism equals the preimage of the submonoid under the
    inverse. -/
@[to_additive "The image an `add_submonoid` under an isomorphism equals the preimage of the `add_submonoid` under the inverse."]
lemma map_eq_comap (e : M ≃* N) :
  e.to_monoid_hom.map S = e.symm.to_monoid_hom.comap S :=
submonoid.ext' $ e.to_equiv.image_eq_preimage S

/-- Given monoids `M, N`, a submonoid `T` of `N` is contained in the image of a submonoid `S` of
    `M` under an isomorphism iff the preimage of `T` under the inverse is contained in `S`. -/
@[to_additive "Given monoids `M, N`, an `add_submonoid` `T` of `N` is contained in the image of an `add_submonoid` `S` of `M` under an isomorphism iff the preimage of `T` under the inverse is contained in `S`."]
lemma le_map_iff_map_le [monoid M] [monoid N]
