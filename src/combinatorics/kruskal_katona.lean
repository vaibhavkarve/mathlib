  ...
  begin
    ...
    { apply h₂, refine ⟨⟨_, _, _, _, _⟩, card_erase_lt_of_mem Hx⟩,
      ...
      { apply lt_of_le_of_lt (max'_le (erase U x) _ _
        ...
        rw mem_erase, refine ⟨(min_ne_max_of_card _ _ _).symm, max'_mem _ _⟩,
        rwa ← same_size } },
    rw ← card_pos at Uh,
...
begin
  ...
  have q := card_le_of_subset this,
    rw [card_powerset, card_univ, card_fin] at q,
  ...
  rw [sum_const_on_nat this, ← nat.le_div_iff_mul_le' zero_lt_two] at q,
...
begin
  ...
  have z: 𝒜bar.card > nat.choose (n-1) (n-r),
    convert size using 1, rw card_image_of_inj_on, intros A _ B _ k,
      replace k := sdiff_partially_injective k, simp [ext] at k, rwa ext,
    apply choose_symm', rw [← nat.add_sub_assoc ‹r ≥ 1›,
                            nat.sub_add_cancel ‹r ≤ n›],
  -- and everything in 𝒜bar has size n-r.
  have: all_sized 𝒜bar (n - r),
    intro A, rw mem_image, rintro ⟨B, Bz, rfl⟩,
    rw [card_univ_diff, card_fin, h₂ _ Bz],
  ...
  apply not_le_of_lt this, convert number_of_fixed_size _, rw card_fin,
